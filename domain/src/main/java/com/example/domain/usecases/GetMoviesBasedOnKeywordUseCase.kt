...
interface GetMoviesBasedOnKeywordUseCase {
    fun getMoviesBasedOnKeyword(currentPage: Int, keyword: String): Flow<Resource<ListMovies>>
}

internal class GetMoviesBasedOnKeywordUseCaseImpl @Inject constructor(
    ...
    override fun getMoviesBasedOnKeyword(
        currentPage: Int,
        keyword: String
    ): Flow<Resource<ListMovies>> = flow {
        try {
            emit(Resource.Loading())
            val movies = repository.getMoviesBasedOnKeyword(currentPage, keyword).mapToModel()
            emit(
